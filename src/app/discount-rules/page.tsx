      ...
      <main className="mx-auto max-w-7xl space-y-6 px-4 py-7 sm:px-6">
        ...
        <section className="rounded-xl border border-slate-600/60 bg-[#232a34] p-6 shadow-2xl shadow-black/20 sm:p-8">
          ...
          <div className="mt-8 grid grid-cols-1 gap-4 sm:grid-cols-3">
            <div className="rounded-lg border border-slate-700/60 bg-[#1c222b] p-4">
              ...
              <div className="mt-2 text-2xl font-bold text-white">{rules?.length ?? "—"}</div>
            ...
            <div className="rounded-lg border border-slate-700/60 bg-[#1c222b] p-4">
              ...
              <div className="mt-1 text-xs text-slate-400">Enforced</div>
            </div>
            <div className="rounded-lg border border-slate-700/60 bg-[#1c222b] p-4">
              <div className="flex items-center justify-between text-xs font-medium text-slate-400">
              ...
              <div className="mt-1 text-xs text-slate-400">vs {(rules ?? []).filter((r) => r.scope === "CATEGORY").length} category rules</div>
            </div>
          </div>
        </section>
        ...
        {showResolver && (
          <section className="rounded-xl border border-amber-500/30 bg-amber-500/5 p-6">
            <h3 className="mb-4 text-sm font-bold text-amber-300 uppercase">Ceiling Resolver</h3>
            ...
            {resolveResult && (
              <div className="mt-4 rounded-lg border border-amber-400/30 bg-[#1c222b] p-4 space-y-1">
                <p className="text-xs text-slate-300">
                  <span className="font-semibold text-white">Allowed Discount:</span>{" "}
                ...
                <p className="text-xs text-slate-400">Limiting scope: {resolveResult.limitingScope}</p>
              </div>
            )}
          </section>
        )}
        ...
        <section className="rounded-xl border border-slate-700/60 bg-[#232a34] p-6 shadow-xl">
          {loading ? (
            ...
            <div className="py-12 text-center text-sm text-slate-400">No discount rules configured.</div>
          ) : (
              ...
              <Table>
                <TableHeader>
                  <TableRow className="border-slate-700 hover:bg-transparent">
                    ...
                    <TableHead className="text-slate-300">Tier / Category</TableHead>
                    <TableHead className="text-slate-300">Max Discount</TableHead>
                ...
                <TableBody>
                  {(rules ?? []).map((rule) => (
                    <TableRow key={rule.id} className="border-slate-800 transition-colors hover:bg-slate-800/50">
                      <TableCell>
                        <Badge className={`border text-xs ${rule.scope === "TIER" ? "border-purple-400/30 bg-purple-400/10 text-purple-200" : "border-cyan-400/30 bg-cyan-400/10 text-cyan-200"}`}>
                          {rule.scope}
                        </Badge>
                      </TableCell>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
          <Card className="w-full max-w-lg border-slate-700 bg-[#1c222b] text-slate-100 shadow-2xl">
            <CardHeader className="border-b border-slate-700/60 pb-4">
              <CardTitle className="text-lg font-bold text-white">Create Discount Rule</CardTitle>
            </CardHeader>
            <CardContent className="pt-4">
              <form onSubmit={handleCreate} className="space-y-4">
                ...
                {newScope === "TIER" ? (
                  ...
                  <div className="space-y-1.5">
                    <Label className="text-xs text-slate-300">Product Category</Label>
